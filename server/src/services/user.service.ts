...
export class UserService {
  async createUser(data: CreateUserDTO) {
    ...
    if (usernameCheck) throw new HttpError(403, "fullname already in use");
  ...
  async updateUser(userId: string, data: UpdateUserDTO) {
    ...
    if (data.fullName && user.fullName !== data.fullName) {
      const usernameExists = await userRepository.getUserByUsername(data.fullName);
      if (usernameExists) {
        throw new HttpError(409, "fullbame already exists");
      }
    }
